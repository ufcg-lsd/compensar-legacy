  ...
  .controller(
    ...
    function (
        ...
        if (selection.rangeCount > 0 && !window.getSelection().isCollapsed) {
          ...
          for (let p of parents) {
            if (isOrContains(p, range.commonAncestorContainer)) {
              ...
              $rootScope.avaliacao.maisInfo[p.getAttribute("comp")] =
                p.innerHTML;
              return;
            }
          }
      ...
      $scope.sendQuestionSubjective = function (avaliacao) {
        ...
        $http
          .post(host + "questao", questaoSubj, AuthService.getAuthorization())
          .then(
            function (response) {
      ...
      $scope.getAvaliacao = () => {
        ...
        return {
          ...
          observacao: $rootScope.avaliacao.obsAvaliacao,
      ...
      $scope.sendQuestionObjective = function (avaliacao) {
        ...
        $http
          .post(host + "questao", questaoObj, AuthService.getAuthorization())
          .then(
            function (response) {
              Notification.success("Questão criada com sucesso!");
              $location.path("/buscas");
      ...
      $scope.checkPasso2 = function (passo) {
        if ($scope.enunciado === null || $scope.enunciado.trim() === "") {
          ...
          // TODO: Comentei esse trecho porque o classificador está indisponível no momento. Quando estiver on, descommitar!!
      ...
      $scope.checkPasso3 = function (passo) {
        ...
        if (passo === "anterior") {
          ...
          $scope.tipo === "Objetiva" &&
          ((typeof $scope.corretas.Value1 === "undefined" &&
            typeof $scope.corretas.Value2 === "undefined" &&
            typeof $scope.corretas.Value3 === "undefined" &&
            typeof $scope.corretas.Value4 === "undefined" &&
            typeof $scope.corretas.Value5 === "undefined") ||
            $scope.alternativas.alternativa1 === "" ||
            $scope.alternativas.alternativa2 === "" ||
            $scope.alternativas.alternativa3 === "" ||
            $scope.alternativas.alternativa4 === "" ||
            $scope.alternativas.alternativa5 === "")
        ) {
          $scope.inputError = true;
        } else if (
          $scope.tipo === "Subjetiva" &&
          ($scope.resposta.espelho === "" ||
            $scope.resposta.espelho === null ||
            $scope.resposta.espelho === "undefined")
        ) {
          $scope.inputError = true;
        } else {
          $scope.nextStep();
